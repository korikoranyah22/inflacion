# Handover incremental para el rediseño · cambios posteriores a v148

Fecha de corte: 22 de agosto de 2026.  
Dashboard fuente: `index.html`.  
Snapshot vigente: `data/dashboard_kawaii_149_fuentes_normalizadas.html`.

## Cómo usar este documento

Tomá este archivo como prompt incremental para portar al dashboard rediseñado todo lo realizado **después** de `data/derivados/AUDITORIA_ESCALAS_LOG_Y_JERARQUIA_VISUAL_V148.md`.

No reemplaza los handovers anteriores del Péndulo, feedback de Claude, EMAE, morosidad, rutas o turismo. Sólo documenta el delta posterior a v148. En caso de conflicto con la auditoría v148, **este documento tiene precedencia** en las reglas de escala de `Poder adquisitivo` y `Tasas e inflación`.

## 1. Corrección obligatoria a la auditoría v148

La auditoría v148 decía que `Poder adquisitivo` y `Tasas e inflación` permanecían exclusivamente lineales y que una vista Log nunca ocultaba series. Esa regla fue revisada.

Ahora ambos gráficos ofrecen dos versiones opcionales:

- `Lineal`: lectura completa de niveles, diferenciales y saldos.
- `Log`: lectura proporcional de las series estrictamente positivas y comparables en ese eje. En `Tasas e inflación`, las series monetarias con signo permanecen visibles en un segundo eje lineal.

La vista inicial sigue siendo siempre `Lineal`. No se transforman los datos, no se suman constantes, no se reemplazan ceros y el tiempo permanece lineal. En `Poder adquisitivo` se filtran sólo las trazas incompatibles en la presentación Log; en `Tasas e inflación` no se oculta ninguna traza.

La interfaz debe explicar de manera visible qué eje cambia de escala y qué ocurre con las series incompatibles. No alcanza con cambiar u ocultar contenido silenciosamente.
...
- Muestra únicamente las series positivas de índices reales.
- Oculta las trazas cuyo `yaxis` es `y2`, es decir, los saldos acumulados con signo.
- Oculta el eje derecho y sus anotaciones; no elimina los datos.
...
- Texto visible del estado: `Vista Log: muestra los índices positivos; los saldos acumulados del eje derecho quedan ocultos en esta versión.`
...
- Cambiar base, cambiar foco, redimensionar la ventana o volver a abrir el tab no debe restaurar accidentalmente las trazas incompatibles mientras Log está activo.
- Volver a Lineal debe restaurar las trazas, el eje derecho y sus anotaciones.
...
- En `Poder adquisitivo`, la ayuda aclara qué saldos con signo se ocultan en Log y reaparecen en Lineal.

...
OUTPUT_HTML = ROOT / "data" / "dashboard_kawaii_138_pendulo_promedios_politicos.html"
...
CSS = r"""
<style id="pendulo-distributivo-v138">
/* v138 · péndulo distributivo con promedios por gobierno y lectura política numérica */
...
.pend-average-control{display:flex;align-items:center;gap:9px;margin:11px 0 0;padding:9px 11px;border:1px dashed #ccb9d8;border-radius:13px;background:#fcf9ff;font-size:9.5px;line-height:1.4;color:#705778}.pend-average-control input{width:17px;height:17px;margin:0;accent-color:#80539a}.pend-average-control b{color:#573765}.pend-average-control span{color:#8b758f}
.pend-political{margin-top:14px;padding:15px;border:1px solid #ddcce8;border-radius:18px;background:linear-gradient(135deg,#fcf9ff,#fffdf6)}.pend-political-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.pend-political-head h4{margin:0;color:#5b3968;font-size:14px}.pend-political-head p{margin:4px 0 0;font-size:9.5px;line-height:1.45;color:#78647e}.pend-political-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;margin-top:11px}.pend-gov-number{padding:13px;border:1px solid #dfd2e6;border-radius:15px;background:rgba(255,255,255,.94)}.pend-gov-number h5{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:0 0 8px;color:#5c3d69;font-size:11.5px}.pend-gov-number h5 span{font-size:8px;color:#8b778f;text-transform:uppercase}.pend-gov-pair{display:grid;grid-template-columns:1fr 1fr;gap:7px}.pend-gov-metric{padding:8px;border-radius:10px;background:#f8f4fb}.pend-gov-metric small{display:block;font-size:7.5px;font-weight:950;text-transform:uppercase;color:#89758d}.pend-gov-metric strong{display:block;margin-top:3px;font-size:16px;color:#5a3c67}.pend-gov-number .reading{display:block;margin-top:8px;padding-top:7px;border-top:1px dashed #e1d6e7;font-size:9px;line-height:1.45;color:#75617a}.pend-political-summary{margin-top:9px;padding:10px 12px;border-left:4px solid #9f74b6;border-radius:11px;background:#fff;font-size:9.8px;line-height:1.5;color:#69536f}.pend-political-summary b{color:#543360}
...
@media(max-width:1050px){.pend-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.pend-grid-2,.pend-verdict,.pend-source-grid{grid-template-columns:1fr}.pend-contrast-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.pend-plain-grid{grid-template-columns:1fr}.pend-component-guide{grid-template-columns:1fr}.pend-political-grid{grid-template-columns:1fr}}
@media(max-width:760px){.pend-card{padding:15px;border-radius:20px}.pend-head{display:block}.pend-head h2{font-size:23px}.pend-controls{grid-template-columns:1fr}.pend-contrast-grid{grid-template-columns:1fr}.pend-chart{min-width:760px;height:470px}.pend-chart.medium{height:410px}.pend-chart.compact{height:370px}.pend-quote{font-size:13px}.pend-btn{padding:8px 10px}.pend-kpi strong{font-size:22px}.pend-everyday-head,.pend-political-head{align-items:flex-start;flex-direction:column}.pend-hundred-flow{grid-template-columns:1fr;gap:23px}.pend-hundred-piece:not(:last-child)::after{content:'⬇';right:auto;left:50%;top:auto;bottom:-22px;transform:translateX(-50%);font-size:16px}}
...
SECTION = r"""
  <!-- PENDULO_DISTRIBUTIVO_TAB_VERSION:3 · promedios políticos numéricos -->
  <section id="tab-pendulo" class="tab-panel">
    <div class="pend-shell">
      ...
      <section class="pend-card">
        ...
        <div class="pend-chart-stickers"><span class="pend-chart-sticker up">⬆️ sube = trabajo/hogares gana participación</span><span class="pend-chart-sticker down">⬇️ baja = EEB gana participación</span><span class="pend-chart-sticker warn">⚠️ participación ≠ plata en el bolsillo</span><span class="pend-chart-sticker">━ promedio observado de cada gobierno</span></div>
        <label class="pend-average-control"><input id="pendAverageToggle" type="checkbox" checked><b>Mostrar líneas de promedio por gobierno</b><span>Se calculan sólo dentro de cada tramo metodológico; una gestión con un único dato no recibe línea.</span></label>
        ...
        <div class="pend-political"><div class="pend-political-head"><div><span class="pend-sticker">🏛️ Política en números</span><h4>Últimas tres gestiones · mismo tramo metodológico</h4><p>Promedio = posición habitual durante la ventana observada. Cambio = diferencia entre el primer y el último trimestre de esa gestión.</p></div><span class="pend-chart-sticker warn">resultado observado ≠ causalidad probada</span></div><div id="pendPoliticalNumbers" class="pend-political-grid"></div><div id="pendPoliticalSummary" class="pend-political-summary">Calculando comparación política…</div></div>
      </section>
...
SCRIPT_TEMPLATE = r"""
<script id="pendulo-distributivo-script-v138">
const PEND_DATA=__PEND_DATA__;
const PEND_MANDATES=__PEND_MANDATES__;
const PEND_CSVS=__PEND_CSVS__;
let pendUniverse='private',pendPerspective='index',pendShowAverages=true,pendRendered=false;
...
function pendShortMandate(name){return {'Carlos Menem':'Menem','Fernando de la Rúa':'De la Rúa','Néstor Kirchner':'N. Kirchner','Mauricio Macri':'Macri','Alberto Fernández':'Alberto','Javier Milei':'Milei'}[name]||name}
function pendGovernmentAverages(rows){const shapes=[],annotations=[];pendStats().filter(s=>s.observations>1&&Number.isFinite(Number(s.average))).forEach(s=>{const rr=rows.filter(r=>r.president===s.mandate);if(rr.length<2)return;const x0=rr[0].date,x1=rr.at(-1).date,mid=new Date((new Date(x0).getTime()+new Date(x1).getTime())/2).toISOString().slice(0,10),modern=rr[0].segment==='modern',color=modern?'rgba(181,126,42,.86)':'rgba(115,92,129,.76)';shapes.push({type:'line',xref:'x',yref:'y',x0,x1,y0:s.average,y1:s.average,line:{color,width:2.5,dash:'dash'},layer:'above'});annotations.push({xref:'x',yref:'y',x:mid,y:s.average,yshift:10,text:`prom. ${pendShortMandate(s.mandate)} <b>${pendSigned(s.average)}</b>`,showarrow:false,bgcolor:'rgba(255,255,255,.9)',bordercolor:color,borderpad:3,font:{size:8,color:'#624b69'}})});return{shapes,annotations}}
function renderPendMain(){
 ...
 if(pendPerspective==='index'){
  ...
  if(pendShowAverages){const averages=pendGovernmentAverages(rows);layout.annotations.push(...averages.annotations);layout.shapes.push(...averages.shapes)}
 ...
 const avgToggle=document.getElementById('pendAverageToggle');if(avgToggle){avgToggle.disabled=pendPerspective!=='index';avgToggle.closest('label').style.opacity=pendPerspective==='index'?'1':'.55'}
...
function renderPendNarrative(){
 ...
 const recent=[macri,alberto,milei];document.getElementById('pendPoliticalNumbers').innerHTML=recent.map(s=>{const avgReading=s.average>=0?`RTA + IMB promediaron ${pendFmt(Math.abs(s.average))} pp por encima de EEB.`:`EEB promedió ${pendFmt(Math.abs(s.average))} pp por encima de RTA + IMB.`;const changeReading=s.change>=0?`Entre extremos se movió ${pendFmt(Math.abs(s.change))} pp hacia trabajo/hogares.`:`Entre extremos se movió ${pendFmt(Math.abs(s.change))} pp hacia EEB.`;return `<article class="pend-gov-number"><h5>${s.mandate}<span>${s.observations} trimestres${s.mandate==='Javier Milei'?' · en curso':''}</span></h5><div class="pend-gov-pair"><div class="pend-gov-metric"><small>Promedio</small><strong>${pendSigned(s.average)}</strong></div><div class="pend-gov-metric"><small>Cambio inicio → fin</small><strong class="${s.change>=0?'pend-move-work':'pend-move-capital'}">${pendSigned(s.change)}</strong></div></div><span class="reading"><b>${pendSigned(s.start)} → ${pendSigned(s.end)}.</b> ${avgReading} ${changeReading}</span></article>`}).join('');
 const towardWork=[...recent].sort((a,b)=>b.change-a.change)[0],towardCapital=[...recent].sort((a,b)=>a.change-b.change)[0];document.getElementById('pendPoliticalSummary').innerHTML=`<b>Comparación política, en números:</b> dentro del tramo moderno comparable, el mayor giro observado hacia trabajo/hogares fue ${towardWork.mandate} (${pendSigned(towardWork.change)} pp) y el mayor giro hacia EEB fue ${towardCapital.mandate} (${pendSigned(towardCapital.change)} pp). Son diferencias entre el primer y el último trimestre asignado a cada gestión: describen el resultado, no prueban por sí solas quién lo causó.`;
...
document.getElementById('pendAverageToggle')?.addEventListener('change',e=>{pendShowAverages=e.target.checked;renderPendMain()});

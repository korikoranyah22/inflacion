...
OUTPUT_HTML = ROOT / "data" / "dashboard_kawaii_136_pendulo_distributivo.html"
...
CSS = r"""
<style id="pendulo-distributivo-v136">
/* v136 · péndulo distributivo · hereda tipografía, fondo y paleta del dashboard */
...
@media(max-width:1050px){.pend-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.pend-grid-2,.pend-verdict,.pend-source-grid{grid-template-columns:1fr}.pend-contrast-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
@media(max-width:430px){.pend-card{padding:12px}.pend-kpis{grid-template-columns:1fr}.pend-chart{min-width:720px}.pend-sub,.pend-reading{font-size:10.5px}.pend-verdict>div{padding:12px}}
...
SECTION = r"""
  <!-- PENDULO_DISTRIBUTIVO_TAB_VERSION:1 -->
...
SCRIPT_TEMPLATE = r"""
<script id="pendulo-distributivo-script-v136">
...
function renderPendMain(){const rows=pendRows();let traces=[],layout;if(pendPerspective==='index'){traces=[pendSegmentTrace(rows,'historical','pendulo','Histórica anual 1993–2007','#a77ac1'),pendSegmentTrace(rows,'modern','pendulo','Moderna trimestral 2016–2026-T1','#5d43a4')];layout=pendLayout('Índice del péndulo · segmentos no empalmados','índice · −100 a +100',{yaxis:{title:'− capital societario · índice · trabajo/hogares +',range:[-100,100],dtick:20,gridcolor:'#eadff0',zeroline:true,zerolinecolor:'#765b81'}})}else if(pendPerspective==='shares'){['historical','modern'].forEach((segment,idx)=>{traces.push(pendSegmentTrace(rows,segment,'share_households',idx?'Trabajo/hogares · moderno':'Trabajo/hogares · histórico','#328d70',idx===0));traces.push(pendSegmentTrace(rows,segment,'share_capital',idx?'EEB · moderno':'EEB · histórico','#c14d78',idx===0))});layout=pendLayout('Participación normalizada','% de RTA + IMB + EEB',{yaxis:{title:'porcentaje normalizado',range:[0,100],dtick:10,gridcolor:'#eadff0'}})}else{const groups=[...new Set(rows.map(r=>r.president))].filter(n=>n!=='Sin serie comparable');groups.forEach((name,idx)=>{const rr=rows.filter(r=>r.president===name);traces.push({x:rr.map(r=>r.date),y:rr.map(r=>r.change_since_mandate_start),customdata:rr.map(pendCustom),name,mode:'lines+markers',showlegend:false,connectgaps:false,line:{color:idx%2?'#b34f7a':'#5f48a4',width:2.6},marker:{size:5},hovertemplate:'<b>%{customdata[0]}</b> · %{customdata[1]}<br>Cambio desde primer dato del mandato: <b>%{y:+.1f}</b> puntos<br>Péndulo del período: <b>%{customdata[7]}</b><extra></extra>'})});layout=pendLayout('Cambio desde el primer dato observado de cada mandato','puntos del índice',{yaxis:{title:'cambio dentro del mandato',gridcolor:'#eadff0',zeroline:true,zerolinecolor:'#765b81'}})}Plotly.react('pendMainChart',traces,layout,PEND_CONFIG)}

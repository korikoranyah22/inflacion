...
evidence_path = HERE / "HISTORICAL_EVIDENCE_COVERAGE_V119.csv"
evidence = read_csv(evidence_path)
for row in evidence:
    if row["episode"] == "E0_2001_2003" and row["variable_family"] == "state_bcra":
        row.update(
            {
                "quality": "PRIMARY_BUYBACK_STRIP_EXACT_ACCOUNT_ROUTE_CUSTODIAN_GAP_DOCUMENTED_REQUEST_ARCHIVAL_SCOPE_HARDENED",
                "gap": "Liquidaciones, blotter BNA, respuesta AGN y registros custodiales siguen abiertos; seis pedidos cubren sistemas heredados, migraciones y disposición, pero no fueron enviados.",
                "next_action": "Presentar sólo con autorización expresa y evaluar toda inexistencia contra los catorce controles V119; mientras tanto no fingir plazos ni respuestas.",
            }
        )
write_csv(evidence_path, evidence)


queue_path = HERE / "HISTORICAL_SOURCE_QUEUE_V119.csv"
queue = read_csv(queue_path)
for row in queue:
    if row["episode"] == "E0_2001_2003" and row["variable_family"] == "state_bcra":
        row.update(
            {
                "status": "INSTITUTIONAL_REQUEST_ARCHIVAL_SCOPE_HARDENED_NOT_SENT",
                "why": "Seis borradores separan Tesoro, BCRA/CRyL, BNA, AGN, CNV y Caja y trazan 46 objetos con alcance histórico, migraciones, copias y disposición; no existe presentación ni respuesta.",
...
inherited = [
    {"script": "qa_v97.py", "pre_v119_result": "EXPECTED_SUPERSEDED_ASSERTION", "post_v119_result": "EXPECTED_SUPERSEDED_ASSERTION", "interpretation": "V97 exige que una fuente recuperada después permanezca sin ruta/hash."},
    *({"script": f"qa_v{i}.py", "pre_v119_result": "PASS", "post_v119_result": "PASS", "interpretation": "Compatible"} for i in (98, 100, 101, 102, 103, 104, 105, 106)),
    *({"script": f"qa_v{i}.py", "pre_v119_result": "EXPECTED_SUPERSEDED_ASSERTION", "post_v119_result": "EXPECTED_SUPERSEDED_ASSERTION", "interpretation": "Congela un checkpoint o conteo anterior."} for i in range(107, 119)),
    {"script": "qa_v119.py", "pre_v119_result": "N/A", "post_v119_result": "PASS", "interpretation": "Sistemas temporales, retención, suficiencia negativa, pedidos no enviados e invariantes actuales."},
]
write_csv(HERE / "INHERITED_QA_STATUS_V119.csv", inherited)
...
for stem in (
    ...
    shutil.copyfile(AUDIT / f"{stem}_V118.csv", AUDIT / f"{stem}_V119.csv")
...
write_csv(AUDIT / "GITHUB_FILE_SIZE_AUDIT_V119.csv", size_rows)
...
completeness.update(
    {
        "checkpoint": "V119",
        "date": "2026-08-29",
        "state": "E0_INSTITUTIONAL_REQUEST_ARCHIVAL_SCOPE_HARDENED_NOT_SENT",
        "numeric_v119_strict_changed": False,
        "sources_newly_preserved_v119": 0,
        "e0_primary_sources_newly_preserved_v119": 0,
        ...
        "historical_workstream": "E0_INSTITUTIONAL_REQUEST_ARCHIVAL_SCOPE_HARDENED_NOT_SENT",
    }
)
completeness.pop("numeric_v118_strict_changed", None)
completeness.pop("sources_newly_preserved_v118", None)
completeness.pop("e0_primary_sources_newly_preserved_v118", None)
(AUDIT / "CURRENT_SOURCE_COMPLETENESS_V119.json").write_text(
    json.dumps(completeness, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
)


backup = REPO / "BACKUP_ACTUALIZACION_2026-08-29.md"
marker = "## V119 · alcance archivístico endurecido"
old_backup = backup.read_text(encoding="utf-8-sig")
if marker not in old_backup:
    old_backup += (
        f"\n\n{marker}\n\n"
        "- Los seis borradores distinguen sistemas históricos, migraciones, reproducciones y disposición concreta.\n"
        "- Se trazaron 46 objetos, 6 rutas temporales, 10 reglas archivísticas y 14 controles de respuesta negativa.\n"
...
def write_checkpoint_manifest() -> None:
    ...
    for path in sorted(HERE.iterdir()):
        if not path.is_file() or path.name == "MANIFEST_V119.json":
            continue
        files.append({"path": path.name, "bytes": path.stat().st_size, "sha256": sha256(path)})
    manifest = {
        "checkpoint": "V119",
        "parent_checkpoint": "V118",
    ...
    (HERE / "MANIFEST_V119.json").write_text(
...
global_manifest = {
    ...
    "exact_entities": 30,
    "closed_network_gate": "NO",
    "source_audit": f"{len(catalog)} master entries; {physical} physical local copies with {hash_ok}/{physical} SHA-valid; no new historical source; six archival-scope-hardened institutional requests drafted and none submitted.",
    "historical_workstream": "E0 institutional request archival scope hardened; Treasury/BCRA/BNA/AGN/CNV/Caja records remain open and no request has been submitted",
